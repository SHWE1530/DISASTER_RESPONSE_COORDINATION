...
def generate_markdown_report(urg_report, urg_cm, error_log, robust_results):
    ...
    report_lines = [
        ...
        "\n## 1. Executive Summary",
        f"- **Overall Urgency Accuracy:** {urg_report['accuracy']:.2%} " if urg_report else "N/A",
        f"- **Robustness Tests Passed:** {sum([1 for r in robust_results if r['passed']])}/{len(robust_results)}",
...
def main():
    ...
    try:
        ...
        # Patch for sklearn version incompatibilities without retraining
        if not hasattr(artifacts["urgency_model"], "multi_class"):
            artifacts["urgency_model"].multi_class = "multinomial"
        if not hasattr(artifacts["hazard_model"], "multi_class"):
            artifacts["hazard_model"].multi_class = "multinomial"
    except Exception as e:
